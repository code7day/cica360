...
>
> Última actualización: 2026-09-17 (**Animación de entrada sitewide: motor `data-reveal` (reemplaza `properties.animation`, ver ADR-005)** — pedido del Tech Lead pivotó de "usar el selector de animación de Filament" a "quitarlo de Filament, animación estática por frontend, aplicada en todos los componentes de todas las páginas del sitio". Motor `[data-reveal]`/`IntersectionObserver` construido y cableado en los 18 componentes de `blocks/`, en `servicios/[slug].astro` (sin tocar header/wave/banderas) y en `blog/*`. Ver CURRENT_STATE.md/PROGRESS.md/DECISIONS.md (ADR-005) para el detalle completo. **Constraint activo: no se hizo git commit/push ni deploy — solo para probar en local.** Pendiente: validación visual del Tech Lead con `astro dev`. Owner: libre.)
